...
import static oop.inner_anonymous.service.ActivityDashboard.*;
...
public class SprintLogApp {
    ...
    public static void main(String[] args) {
        ...
        LearningActivity[] activities = {
                new LectureLog("Java enum 기초", 50, PUBLIC, "박코치"),
                new LectureLog("Java 내부클래스", 70, PUBLIC, "김선생"),
                new PracticeLog("예외 처리 실습", 80, PUBLIC, 90),
                new PracticeLog("인터페이스 실습", 100, PUBLIC, 85),
                new ReadingLog("객체지향의 사실과 오해", 35, PRIVATE, "조영호")
        };

        // ── 2. 정적 중첩 클래스 시연: Summary ──────────────────────────────────────────
        // Summary는 ActivityDashboard 객체 없이도 생성이 가능하지만, Summary가 의미있는 활성 댇체 수를 가지게 하기 위해
        // dashboard.summarize()를 호출해서 리턴 받음.
        ActivityDashboard dashboard = new ActivityDashboard(activities);
        Summary summary = dashboard.summarize();
        ...
        System.out.println();
        ActivityPrinter activityPrinter = activity -> {
        ...
        dashboard.new ReportBuilder(activityPrinter).print();
    }
}
